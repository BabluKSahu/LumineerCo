...
export function AdminDashboard() {
  ...
  const statCards = [
    { label: 'Total Leads', value: stats.totalLeads, icon: Users, color: 'text-blue-400', change: '+12%' },
    { label: 'Active Projects', value: stats.activeProjects, icon: Briefcase, color: 'text-purple-400', change: '+3' },
    { label: 'Completed', value: stats.completedProjects, icon: CheckCircle, color: 'text-green-400', change: '+8' },
    { label: 'Revenue', value: `₹${stats.totalRevenue.toLocaleString()}`, icon: DollarSign, color: 'text-amber-400', change: '+18%' },
    { label: 'Conversion', value: `${stats.conversionRate}%`, icon: TrendingUp, color: 'text-cyan-400', change: '+5%' },
    { label: 'Avg Project', value: `₹${stats.avgProjectValue.toLocaleString()}`, icon: BarChart3, color: 'text-pink-400', change: '+7%' },
  ]

  return (
    <div className="min-h-screen bg-gray-950">
      {/* Sidebar */}
      <aside className="fixed inset-y-0 left-0 w-64 bg-gray-900 border-r border-gray-800 z-40 hidden lg:block">
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
            <a href="#overview" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800 transition-colors">
              <LayoutDashboard className="w-5 h-5" />
              Overview
            </a>
            <a href="#projects" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800 transition-colors">
              <Briefcase className="w-5 h-5" />
              Projects
            </a>
            <a href="#agents" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800 transition-colors">
              <Bot className="w-5 h-5" />
              Agents
            </a>
            <a href="#analytics" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800 transition-colors">
              <BarChart3 className="w-5 h-5" />
              Analytics
            </a>
            <a href="#settings" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-300 hover:text-white hover:bg-gray-800 transition-colors">
          ...
          <div className="p-4 border-t border-gray-800 space-y-2">
            ...
            <Button variant="ghost" className="w-full justify-start gap-3 text-red-400 hover:bg-red-500/10">
        ...
        <div className="p-6 md:p-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-400 mt-1">Real-time overview of your AI-powered studio</p>
            </div>
            <div className="flex items-center gap-3">
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-800">
                        <th className="text-left p-4 font-medium text-gray-400 text-sm">Project</th>
                        <th className="text-left p-4 font-medium text-gray-400 text-sm hidden md:table-cell">Client</th>
                        <th className="text-left p-4 font-medium text-gray-400 text-sm hidden lg:table-cell">Service</th>
                        <th className="text-left p-4 font-medium text-gray-400 text-sm">Status</th>
                        <th className="text-left p-4 font-medium text-gray-400 text-sm hidden sm:table-cell">Agent</th>
                        <th className="text-right p-4 font-medium text-gray-400 text-sm">Value</th>
                      </tr>
                    </thead>
                    <tbody>
                      {projects.map((project) => (
                        <tr key={project.id} className="border-b border-gray-800/50 hover:bg-gray-900/50 transition-colors">
                          <td className="p-4">
                            <div className="font-medium text-white">{project.id}</div>
                            <div className="text-xs text-gray-500">{new Date(project.createdAt).toLocaleDateString()}</div>
                          </td>
                          <td className="p-4 hidden md:table-cell text-gray-300">{project.clientName}</td>
                          <td className="p-4 hidden lg:table-cell text-gray-400 text-sm">{project.service}</td>
                          <td className="p-4">
                          ...
                          <td className="p-4 text-right font-medium text-white">₹{project.value.toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  {agents.map((agent) => (
                    <div key={agent.name} className="p-4 flex items-center justify-between">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={cn('w-2.5 h-2.5 rounded-full', agentStatusColors[agent.status])} />
                        <div>
                          <p className="font-medium text-white truncate">{agent.name}</p>
                          <p className="text-xs text-gray-500">{agent.tasksCompleted} tasks • {agent.avgTime} avg</p>
                        </div>
                      </div>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card variant="bordered" className="p-6 text-center hover:border-amber-500/50 transition-colors cursor-pointer">
              ...
              <p className="text-sm text-gray-400">Weekly PDF report</p>
            </Card>
            <Card variant="bordered" className="p-6 text-center hover:border-amber-500/50 transition-colors cursor-pointer">
              <Users className="w-8 h-8 text-amber-400 mx-auto mb-3" />
              <h3 className="font-semibold text-white mb-1">New Client</h3>
              <p className="text-sm text-gray-400">Onboard client</p>
            </Card>
            <Card variant="bordered" className="p-6 text-center hover:border-amber-500/50 transition-colors cursor-pointer">
              <Bot className="w-8 h-8 text-amber-400 mx-auto mb-3" />
              <h3 className="font-semibold text-white mb-1">Deploy Agent</h3>
              <p className="text-sm text-gray-400">Start new task</p>
            </Card>
            <Card variant="bordered" className="p-6 text-center hover:border-amber-500/50 transition-colors cursor-pointer">
